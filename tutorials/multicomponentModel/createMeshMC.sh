...
Image_name="multicomponent_mesh"
...
pore_index_X=0.00000008
pore_index_Y=0.00000002
pore_index_Z=0
...
calc_sci() {
	echo "scale=12; $1" | bc -l | awk '{printf "%.4e\n", $1}'
}
...
then
    ...
    #python stlImgSeg.py --ratio=$ratio
    rm -f $Image_name\.*
    ...
    pore_index_0=$(calc_sci "$pore_index_X*$res")
    pore_index_1=$(calc_sci "$pore_index_Y*$res")
    pore_index_2=$(calc_sci "$pore_index_Z*$res")
...
echo -e "Coordinates at center of a pore = ($pore_index_X,$pore_index_Y,$pore_index_Z) in the cropped image"
...
sed -i "s/poreIndex0/$pore_index_X/g" system/snappyHexMeshDict
sed -i "s/poreIndex1/$pore_index_Y/g" system/snappyHexMeshDict
sed -i "s/poreIndex2/$pore_index_Z/g" system/snappyHexMeshDict
