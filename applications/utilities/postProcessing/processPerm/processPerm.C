...
int main(int argc, char *argv[])
{
	...
	csvfile << "time poro perm Lpore Re UD nuD\n";
	...
	forAll(timeList, timeStep)
	{
		...
		(
			...
			mesh,
		    dimensionedScalar("nuCorrected",dimViscosity,nu.value())
		);
		...
		(
			...
			mesh
                ...
                scalar nuD = nuCorrected.weightedAverage(clip).value();
                //scalar nuD = nuavg*vol/(x2-x1)/(y2-y1)/(z2-z1);
		...
		scalar Re = UD/poro*Lpore/nuD;


		if (Pstream::master())
		{
			
			csvfile << runTime.timeName() << " " << poro << " " << perm << " " << Lpore << " " << Re << " " << UD << " " << nuD << "\n";
		}
	}
